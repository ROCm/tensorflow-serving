...
TF_VERS_TO_POST_INSTALL="2.19.0"
TF_VERS_SUBSCRIPT=$( \
                    basename $ROCM_PATH | \
                    cut -d "-" -f2 | \
                    tr -d "." || \
    ...
    python3 -m pip uninstall -y tensorflow tensorflow-gpu tensorflow-estimator keras keras-nightly && \
    #python3 -m pip install tensorflow-rocm==${TF_VERS_TO_POST_INSTALL}.${TF_VERS_SUBSCRIPT} -f ${TF_REPO} || \
    python3 -m pip install tensorflow-rocm==${TF_VERS_TO_POST_INSTALL} -f ${TF_REPO} || \
